...
BEGIN
	...
	BEGIN
		...
		INTO profileExists 
		FROM DBA_PROFILES 
		WHERE PROFILE = 'DEVICEAPI_SERVICES';
		DBMS_OUTPUT.PUT_LINE ('** Profile creationg steps - &_DATE');
		IF profileExists = 0 THEN
			DBMS_OUTPUT.PUT_LINE ('Creating Profile = DEVICEAPI_SERVICES ...');
			EXECUTE IMMEDIATE
				'CREATE PROFILE DEVICEAPI_SERVICES
		...
		ELSE
			DBMS_OUTPUT.PUT_LINE ('Profile = DEVICEAPI_SERVICES exists, steps ignored');
		END IF;
	EXCEPTION
		WHEN OTHERS THEN
	...
	BEGIN
		...
		IF schemaUserExists = 0 THEN
			...
			EXECUTE IMMEDIATE 'CREATE USER "&1" IDENTIFIED BY "&2" PROFILE DEVICEAPI_SERVICES';
